...
router.post('/get-reset-token/', async (req, res) => {
    ...
    try {
        ...
        let data = {
            ...
            context: {
                url: `http://${HOST}:${3000}/auth/reset-password/?token=${resetCode.code}`,
            }
        };

        smtpTransport.sendMail(data, function (err) {
            if (!err)
                return res.status(200).send({ success: true, message: 'Done! Kindly check your email for further instructions' });
            return res.status(500).send({
                success: false,
                message: 'Server Error'
            });
        ...
        if (!req.files) {
            ...
            try {
                ...
                try {
                    ...
                    const update = await setProfilePicture(token, `../../media/profile-pictures/${userId}${extension}`)
                    if (!update) {
                        return res.status(404).send({
                            success: false,
                            message: 'Invalid Token',
